...
- Check `YZE/REALM_VTT_API_REFERENCE.md` first for any Realm VTT API pattern.
  This is the combined reference compiled from SEAN_CODE_EXAMPLES.md + PTA source files.
- If a method or pattern is not in that document, it is unconfirmed.
  Do not use it. Note it with a TODO and ask Sean directly.
- Secondary sources (if the reference is incomplete): PTA/common.js, PTA/character-main.html.
  Note: PTA has no rollhandlers/ folder and no character-skills/gear HTML files.
