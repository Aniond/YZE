...
const attrCount  = meta.attrCount  || 0;
const skillCount = meta.skillCount || 0;
const gearCount  = meta.gearCount  || 0;
...
let rerollCount = p.rerollCount;

// ── Pride bonus (+1 free success, once per session) ───────────────────────
const prideBonus = !isPush ? parseInt(api.getValue('data.prideBonus') || '0', 10) : 0;
if (prideBonus > 0) {
...
let stressCount  = !isPush ? parseInt(api.getValue('data.stress') || '0', 10) : 0;
...
let stressVals   = [];
if (stressCount > 0) {
  const sr      = yzeRollPoolN(stressCount);
...
if (meta.isHeal && successes > 0) {
  ...
  if (targets.length > 0) {
    const tok   = targets[0].token;
    const curHp = parseInt(api.getValueOnRecord(tok, 'data.curHealth') || '0', 10);
    const maxHp = parseInt(api.getValueOnRecord(tok, 'data.maxHealth') || '1', 10);
    const newHp = Math.min(maxHp, curHp + successes);
...
if (meta.isRepair) {
  const canRestore = Math.min(successes, Math.max(0, (meta.maxBonus || 0) - (meta.curBonus || 0)));
  if (canRestore > 0) {
...
// ── Push eligibility ──────────────────────────────────────────────────────
let canPush = !isPush && rerollCount > 0;
...
if (canPush) {
  ...
  const _sn = String(skillName).replace(/'/g, '');
  const _pm = "{isPush:true,mode:'pool'"
    + ",attrKey:'" + attrKey + "'"
    + ",skillName:'" + _sn + "'"
    + ",opposed:" + (opposed || 0)
