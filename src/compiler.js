...
export async function compile(directory, { verbose = false } = {}) {
  ...
  // For combatTracker lifecycle hooks (onTurnStart, onTurnEnd, onRoundStart,
  // onRoundEnd), inline the referenced file's script content. Authoring uses
  // a path string in config; Realm's API expects the actual script text.
  if (settings.combatTracker && typeof settings.combatTracker === "object") {
    const hookKeys = ["onTurnStart", "onTurnEnd", "onRoundStart", "onRoundEnd"];
    for (const hook of hookKeys) {
      const value = settings.combatTracker[hook];
      if (typeof value === "string" && value.endsWith(".js")) {
        const filePath = join(dir, value);
        if (verbose) console.log(`  Reading combatTracker.${hook}: ${value}`);
        settings.combatTracker[hook] = await readFile(filePath, "utf-8");
      }
    }
  }
